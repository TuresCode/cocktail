...
export default function DatabaseManagement() {
  ...
  const handleInputChange = (
    ...
    setNewCocktail((prev) => ({
      ...prev,
      [name]:
        name === "ingredients" ? value.split(",").map((i) => i) : value,
    }));
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-600 to-blue-500 text-white p-4 sm:p-8">
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
          <h1 className="text-3xl sm:text-4xl font-bold">Database Management</h1>
          <Link
            href="/"
            className="bg-white text-purple-600 px-4 py-2 rounded-md hover:bg-purple-100 transition-colors w-full sm:w-auto text-center"
          >
            Back to Suggester
          </Link>
        </div>
        ...
        >
          ...
          <div className="mb-4">
            <label htmlFor="recipe" className="block mb-2">
              Recipe:
            </label>
            <textarea
              id="recipe"
